...
- 按下 `i` 后，终端会弹出任务共 6 个阶段以及专家**自动判定**的接管分支和
  ...
  `resume_handover`（左臂还抓着杆、右臂需要重新抓）里，专家会先张开右夹爪并把
  右臂退回原点，给重新靠近留出空间，然后才把杆摆回交接位姿并重抓。策略常常停在
  "右臂贴着杆"的状态，不退右臂的话重抓会在杆旁边闭合、夹空。退右臂这一步是准备
  动作：如果它本身规划失败，会回退到原来的流程而不是直接判失败。
