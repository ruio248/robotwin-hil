...
class Base_Task(gym.Env):
    ...
    def _init_task_env_(self, table_xy_bias=[0, 0], table_height_bias=0, **kwags):
        ...
        # RoboTwin historically reserves 50k renderer descriptors for broad
        # asset-heavy tasks.  Keep that default, but allow small tasks and
        # interactive runners to request a smaller descriptor pool.  This
        # changes capacity only; it does not change shaders, camera outputs,
        # or physics.
        self.render_max_num_materials = int(
            kwags.get("render_max_num_materials", 50000)
        )
        self.render_max_num_textures = int(
            kwags.get("render_max_num_textures", 50000)
        )
        if self.render_max_num_materials <= 0 or self.render_max_num_textures <= 0:
            raise ValueError("renderer material/texture capacities must be positive")
        self.data_type = kwags.get("data_type", None)
        self.save_data = kwags.get("save_data", False)
        # Bulk collection keeps RGB in HDF5; MP4 export is opt-in for review.
        self.save_video = kwags.get("save_video", True)
    ...
    def setup_scene(self, **kwargs):
        ...
        set_global_config(
            max_num_materials=self.render_max_num_materials,
            max_num_textures=self.render_max_num_textures,
        )
