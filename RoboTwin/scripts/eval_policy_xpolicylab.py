...
def main(usr_args: dict[str, Any]) -> None:
    ...
    fixed_seed_list = load_fixed_seed_list(usr_args)
    test_num = len(fixed_seed_list) if fixed_seed_list is not None else int(usr_args.get("test_num", 100))
...
def eval_remote_policy(
    ...
    *,
    ...
    succ_seed = 0
    now_seed = st_seed
    fixed_seed_mode = seed_list is not None
    ...
    while succ_seed < test_num:
        if fixed_seed_mode:
            now_seed = int(seed_list[succ_seed])
        render_freq = args["render_freq"]
        args["render_freq"] = 0
        episode_info = {"info": {}}
        ...
        if expert_check:
            try:
                ...
                if fixed_seed_mode:
                    safe_close_env(task_env)
                    raise RuntimeError(f"fixed seed {now_seed} failed expert setup")
                safe_close_env(task_env)
                ...
                if fixed_seed_mode:
                    safe_close_env(task_env)
                    raise RuntimeError(f"fixed seed {now_seed} failed expert setup") from e
                safe_close_env(task_env)
                now_seed += 1
                args["render_freq"] = render_freq
        ...
        if expert_check and not (task_env.plan_success and task_env.check_success()):
            if fixed_seed_mode:
                safe_close_env(task_env)
                raise RuntimeError(f"fixed seed {now_seed} failed scripted-expert check")
            now_seed += 1
            args["render_freq"] = render_freq
            continue
        ...
        try:
            ...
            if fixed_seed_mode:
                safe_close_env(task_env)
                raise RuntimeError(f"fixed seed {now_seed} failed evaluation setup")
            safe_close_env(task_env)
            ...
            if fixed_seed_mode:
                safe_close_env(task_env)
                raise RuntimeError(f"fixed seed {now_seed} failed evaluation setup") from e
            safe_close_env(task_env)
        ...
        if not fixed_seed_mode:
            now_seed += 1

    return now_seed, task_env.suc


def load_fixed_seed_list(usr_args: Mapping[str, Any]) -> list[int] | None:
    manifest_path = usr_args.get("seed_manifest")
    if not manifest_path:
        return None
    split = str(usr_args.get("seed_split", "test")).strip().lower()
    if split not in {"development", "test"}:
        raise ValueError("seed_split must be development or test")
    with open(os.fspath(manifest_path), "r", encoding="utf-8") as f:
        manifest = json.load(f)
    values = manifest.get(f"{split}_seeds")
    if not isinstance(values, list) or not values:
        raise ValueError(f"seed manifest has no non-empty {split}_seeds list")
    seeds = [int(value) for value in values]
    if len(set(seeds)) != len(seeds):
        raise ValueError(f"seed manifest contains duplicate {split} seeds")
    return seeds
...
def build_instruction(
    ...
    *,
    ...
    if (
        ...
        and hasattr(task_env, "source_arm_tag")
        and hasattr(task_env, "receiver_arm_tag")
    ):
        placeholder_info = {
            "{a}": str(task_env.source_arm_tag),
...
def parse_args() -> dict[str, Any]:
    ...
    if args.seed_manifest is not None:
        usr_args["seed_manifest"] = args.seed_manifest
